...
export const addAlert = (id, alert) => {
    ...
    if (!user || !alert?.uuid) return;

    if (!Array.isArray(user.alerts)) user.alerts = [];

    // Deduplicate: if alert for this skin already exists, update channel_id
    const existingIndex = user.alerts.findIndex(a => a?.uuid === alert.uuid);
    if (existingIndex !== -1) {
        user.alerts[existingIndex] = alert;
    } else {
...
export const sendAlert = async (id, account, alerts, expires, tryOnOtherShard = true, alertsLength = alerts?.length || 0) => {
    const user = getClient()?.users.cache.get(id);
    const username = user ? user.username : id;

    // Deduplicate positive alerts before processing to guarantee no duplicate skin embeds/buttons
    alerts = removeDupeAlerts(alerts);
    ...
    if (tryOnOtherShard) {
        ...
        filteredAlerts[alerts[0].channel_id] = alerts;
    }
    ...
    for (const channel_id of Object.keys(filteredAlerts)) {
        ...
        const buttons = [];
        const alertsArray = removeDupeAlerts(filteredAlerts[channel_id]);
        ...
        // Limit embeds to 10 max (Discord API limit)
        if (message.embeds.length > 10) {
            message.embeds = message.embeds.slice(0, 10);
        }

        // Chunk buttons into ActionRows of max 5 buttons (Discord API limit)
        if (buttons.length > 0) {
            for (let i = 0; i < buttons.length && message.components.length < 5; i += 5) {
        ...
        await channel.send(message).catch(async e => {
            console.error(`Could not send alert message in #${channel.name || channel_id}! Do I have the right role?`);
            try {
...
export const sendDailyShop = async (id, shop, channelId, valorantUser, tryOnOtherShard = true) => {
    ...
    await channel.send({
        ...
        console.error(`Could not send daily shop in #${channel.name || channelId}:`, e);
    });
}
